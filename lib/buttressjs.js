...
class Buttress {
   ...
   * @return {Promise}
   */
  async createUserTransientPolicy(userId, policyName, entity) {
    const policy = {
      name: policyName,
      merge: true,
      selection: {
    ...
    await this.User.updatePolicyProperty(userId, {[policyName]: true});
  }
   ...
   * @return {Promise}
   */
  async removeUserTransientPolicy(userId, policyName, policyProperties) {
    await this.User.setPolicyProperty(userId, policyProperties);
