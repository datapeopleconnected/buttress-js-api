...
class Buttress {
  ...
  init(options) {
    ...
    this.options.userRoles = options.userRoles || {};
  ...
  initSchema() {
    ...
    const updateUserRoles = new Promise((resolve, reject) => {
      if (!this.options.userRoles.length) {
        resolve();
      }

      let json = JSON.stringify(this.options.userRoles);
      let url = `${this.url}/app/userRoles`;
      restler
        .put(url, {
          query: {
            token: this.authToken
    ...
    return Promise.all([updateSchema, updateUserRoles]);
  }
