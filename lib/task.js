...
const _getAllReminders = buttressAuthToken => {
  let token = buttressAuthToken ? buttressAuthToken : Buttress.authToken;

  return new Promise((resolve, reject) => {
    let url = `${Buttress.url}/task/reminders`;
    restler.get(url, {query: {token: token}})
      .on('success', data => resolve(data))
      .on('fail', (data, response) => reject(new Error(response.statusMessage)))
      .on('error', err => reject(err));
...
const _load = buttressId => {
  return new Promise((resolve, reject) => {
    let url = `${Buttress.url}/task/${buttressId}`;
...
const _saveAll = details => {
  return new Promise((resolve, reject) => {
    let url = `${Buttress.url}/task/bulk/add`;
    const json = JSON.stringify(details);
    restler
      .post(url, {
        query: {
...
const _update = (buttressId, details) => {
  return new Promise((resolve, reject) => {
    const url = `${Buttress.url}/task/${buttressId}`;
    const json = JSON.stringify(details);
    restler
      .put(url, {
        query: {
...
module.exports = {
  getAll: _getAll,
  getAllReminders: _getAllReminders,
