...
class Schema {
   ...
   * @return {object}
   */
  static create(schema, buttressOptions) {
    if (!schema) {
      return false;
    }
   ...
   */
  static createFromPath(schema, path, buttressOptions) {
    lambda.log('fe eh tyb');
   ...
   */
  static async inflate(schema, createId, buttressOptions) {
   ...
   * @return {*} defaultValue
   */
  static async getPropDefault(config, buttressOptions) {
    let res;
    ...
    case 'string':
      ...
      if (config.__default === 'randomString') {
        ...
        const bytes = (buttressOptions.isolated) ? await lambda.cryptoRandomBytes(length) : cryptoRandomBytes(length);
