...
class Schema {
  ...
  async _request(type, path, options, attempt = 0, redirect = false) {
    ...
    if (this._ButtressOptions.isolated) {
      const response = await asyncRunner('fetch', {
        url,
        options,
      });

      if (options.method === 'POST') {
        const needsRedirect = this._postRedirect(response.res, url);
        if (needsRedirect) {
          lambdaLog(`[WARNING] A POST redirect is occuring due to different http protocol`);
          return this._request(type, path, options, attempt, true);
        }
      }

      if (!response.res.ok) {
        response.data = response.res.body;
