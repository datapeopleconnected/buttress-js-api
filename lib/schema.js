...
class Schema {
   ...
   * @return {promise}
   */
  _request(type, path, options, attempt = 0, redirect = false) {
    if (!this._route) {
    ...
    const url = new URL(`${this.getEndpoint()}/${this._route}`);
    if (path) {
      url.pathname = `${url.pathname}/${path}`;
    }

    Object.keys(options.params)
      .forEach((key) => url.searchParams.append(key, options.params[key]));
    ...
    return fetch(url, options)
      .then((response) => {
        if (options.method === 'POST') {
          ...
          if (originalURL === redirectedURL && originalProtocol !== redirectedProtocol) {
